...
dat_master <- read.csv(file = here::here("data", "lcid", "processed", "masterfile.csv"), header = TRUE)

## ============================================================================================================================ ##
# TIME-INVARIANT MASTERDAT (TVAR) -------------------------------------------------------------------------------------------
## ============================================================================================================================ ##

mod_results <- load_model_results(data_location = "local", timeinvariant = FALSE, folder = "dd_hyperbolic_nonzero")
...
ddtvar %<>%
  rename_with(~str_replace_all(., "_c", ""), contains("age"))
...
ddtvar %<>%
  ...
  select(subjID, contains("logk"), contains("_k"), everything())
...
ddtvar_long <- ddtvar %>%
  ...
  arrange(subjID, wave) %>%
  select(subjID, wave, contains("logk"), contains("_k"), everything())

## Save TVAR processed data --------------------------------------------------------------------------------------------------- ##
write.csv(ddtvar, file = here::here("data", "lcid", "dd_delaydiscount", "processed", "tvar_masterdat_wide.csv"))
write.csv(ddtvar_long, file = here::here("data", "lcid", "dd_delaydiscount", "processed", "tvar_masterdat_long.csv"))

## ============================================================================================================================ ##
# TIME-INVARIANT MASTERDAT (TINVAR) -----------------------------------------------------------------------------------------
## ============================================================================================================================ ##

mod_results_tinvar <- load_model_results(data_location = "local", timeinvariant = TRUE)

dd_hyperbo_params_tinvar <- mod_results_tinvar$dd_hyperbo_tinvar_params$individual_params %>%
  tidyr::pivot_wider(names_from = parameter, values_from = c(estimate, hdi_lower, hdi_upper)) %>%
  dplyr::mutate(subjID = subj_id) %>%
  select(subjID, everything(), -subj_id)

# Remove subjIDs not included in tinvar parameters
ddtinvar <-  dplyr::left_join(dd_hyperbo_params_tinvar, dat_master, by = "subjID") %>%
  #Drop columns that are all NA
  select(where(~!all(is.na(.))))

# Check above remove for correctness
ddtinvar$subjID %in% dd_hyperbo_params_tinvar$subjID %>% table()

# Remove trailing "_c" from age columns
ddtinvar %<>%
  rename_with(~str_replace_all(., "_c", ""), contains("age"))

# Add log-transformed delay discounting parameter k
ddtinvar %<>%
  mutate(
    logk = -log(estimate_k),
    logk_hdi_lower = -log(hdi_lower_k),
    logk_hdi_upper = -log(hdi_upper_k)
  ) %>%
  select(subjID, contains("logk"), contains("_k"), everything())

# create long dataframe for plotting
ddtinvar_long <- ddtinvar %>%
  pivot_longer(
    cols = contains("w0"),
    names_to = c("wave", ".value"),
    names_pattern = "w(\\d+)_(.*)"
...
## Save TINVAR processed data ------------------------------------------------------------------------------------------------- ##
write.csv(ddtinvar, file = here::here("data", "lcid", "dd_delaydiscount", "processed", "tinvar_masterdat_wide.csv"))
write.csv(ddtinvar_long, file = here::here("data", "lcid", "dd_delaydiscount", "processed", "tinvar_masterdat_long.csv"))
...
## Check Age Ranges =========================================================================================================== ##
summary(validate::confront(ddtvar, validate::validator(    # TIME-VARIANT
  r1 = w01_age >= 6.75,
  r2 = w02_age >= 7.75,
  r3 = w03_age >= 8.75,
  r4 = w04_age >= 9.75,
...
summary(confront(ddtinvar, validate::validator(  # TIME-INVARIANT
  r1 = w01_age >= 6.75,
  r2 = w02_age >= 7.75,
  r3 = w03_age >= 8.75,
  r4 = w04_age >= 9.75,
